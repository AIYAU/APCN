...
from utils.sim import top_n_similarities_batch
...
class NoiseSiameseNetwork(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, noise_std=0.1):
        '''
        将输入向量映射到输出，并输出其带噪声的版本
        '''
        super(NoiseSiameseNetwork, self).__init__()
        self.network = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, output_dim)
        )
        self.noise_std = noise_std  # 高斯噪声的标准差
...
class CLIP(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = NoiseSiameseNetwork(input_dim=768, hidden_dim=256, output_dim=768)
...
class CLIP2(nn.Module):
    ...
    def forward(self, f1):
        # print(f"{f1.shape, f2.shape}")
        new_vector, top_tensors, softmax_similarities = top_n_similarities_batch(f1, n = 5)
        return f1 @ new_vector.T
...
class ClipPrototypicalNetworks(nn.Module):
    ...
    def compute_features(self, images: Tensor) -> Tensor:
        ...
        # print(f"图像特征：{centered_features.shape},文本特征：{text_feature.shape}")
        # return centered_features+text_feature
        new_vector, top_tensors, softmax_similarities = top_n_similarities_batch(original_features, n = 3)
        # return torch.pow(self.cs_wt,0.5) * centered_features + torch.pow(1-self.cs_wt,0.5)* new_vector
        # return 0.6 * centered_features + 0.4 * new_vector
